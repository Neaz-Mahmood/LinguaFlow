...
export class FlowSessionsService {
  ...
  async getTodaySession(userId: number): Promise<FlowSession> {
  ...
  async getTodayFullSession(userId: number): Promise<any> {
  ...
  async updateSession(userId: number, data: any): Promise<FlowSession> {
    const session = await this.getTodaySession(userId);
    const user = await this.usersRepository.findOne({ where: { id: userId } });

    if (
      data.comprehensible_input_completed !== undefined &&
    ...
    if (
      data.shadowing_completed !== undefined &&
    ...
    if (
      data.output_completed !== undefined &&
